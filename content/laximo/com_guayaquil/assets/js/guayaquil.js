...
var utils = {
  compare: function (arr1, arr2) {
    if (!(arr1 instanceof Array) || !(arr2 instanceof Array)) {
      throw new TypeError(
        "#compare accepts two parameters, both being Arrays."
      );
    }
...
function initServiceMessages() {
  ...
  if (requestMessageState && requestMessageState === "active") {
    jQuery(document)
      .find('[data-toggle-class=".request_message"]')
      .addClass("active");
  ...
  if (responseMessageState && responseMessageState === "active") {
    jQuery(document)
      .find('[data-toggle-class=".response_message"]')
      .addClass("active");
...
function highLightXml(element) {
  ...
  var responseElem = parser.parseFromString(
    jQuery(".response_message").text(),
    "text/xml"
  );

  if (document.body.getAttribute("data-mark") === "0") {
    if (
      jQuery(responseElem).find("row[oem]").length ||
      jQuery(responseElem).find("Detail[oem]").length
    ) {
...
jQuery(document).ready(function () {
  ...
  var loginContent = jQuery(document)
    .find("#login-form-content div.login-container")
    .first();
...
function checkVinValue(value, submit_btn) {
  ...
  if (expr.test(value)) {
    ...
    if (catalog) {
      window.location =
        "/katalog-laximo?task=vehicles&ft=findByVIN&c=" +
        catalog +
        "&vin=$vin$&ssd=".replace("$vin$", value);
    } else {
      window.location =
        "/katalog-laximo?task=vehicles&ft=findByVIN&c=&vin=$vin$&ssd=".replace(
          "$vin$",
          value
        );
    }
...
function checkFrameValue(frameno, submit_btn) {
  ...
  if (result) {
    window.location =
      "/katalog-laximo?task=vehicles&ft=findByFrame&ssd=&c=" +
      catalog +
      "&frameNo=" +
      frameno;
  }
...
function checkOem(oem, block, ssd) {
  ...
  if (expr.test(value)) {
    ...
    var catalog = form.find('input[name="c"]').val();
    window.location = "/katalog-laximo?" + serrialized;
  } else {
...
function tree_toggle(event, elem) {
  ...
  var newClass = hasClass(node, "qgExpandOpen")
    ? "qgExpandClosed"
    : "qgExpandOpen";
...
QuickGroups.InnerSearch = function (
  value,
  current_path,
  item,
  filtered_groups
  ...
  items.each(function () {
    ...
    if (el.hasClass("qgContent")) {
      ...
      var $detailName = el.find(
        ".qg-detail-names .qg-detail-name:Contains(" + value + "):eq(0)"
      );

      var inName =
        $name.find("a:Contains(" + value + ")").length ||
        $aliases.is(":Contains(" + value + ")");

      if (el.find("a").length && (inName || $detailName.length)) {
        jQuery(
          '<div class="qgFilteredGroup">' +
            '<div class="qgCurrentPath">' +
            current_path +
            "</div>" +
            '<div class="qgFilteredName">' +
            $name.html() +
            (inName ? "" : " / " + $detailName) +
            "</div>" +
            "</div>"
        ).appendTo(filtered_groups);
      }
    ...
    if (items.length == 1) {
      ...
      jQuery.colorbox({
        html: function () {
          ...
          var urls = jQuery(
            'tr[name="' + elName + '"] td[name=c_oem] a.follow'
          );
...
jQuery(document).on("ready load", function () {
  ...
  jQuery(document).on("click", "tr.g_highlight", function () {
    var name = jQuery(this).attr("name");
...
function hl(el, type) {
  ...
  if (glow_name == name) {
    if (type == "in")
      jQuery('.g_highlight_glow[name="' + name + '"]').attr(
        "class",
        "g_highlight_over"
      );
    else
      jQuery('.g_highlight_over[name="' + name + '"]').attr(
        "class",
        "g_highlight_glow"
      );
  } else {
    if (type == "in")
      jQuery('.g_highlight[name="' + name + '"]').attr(
        "class",
        "g_highlight_over"
      );
    else
...
jQuery(document).ready(function ($) {
  jQuery(".guayaquil_floatunitlist_box div").hover(
    function () {
      jQuery('div[name="' + jQuery(this).attr("name") + '"]')
        .parent()
        .addClass("guayaquil_floatunitlist_box_hover");
    },
    function () {
      jQuery('div[name="' + jQuery(this).attr("name") + '"]')
        .parent()
        .removeClass("guayaquil_floatunitlist_box_hover");
    }
  );
  ...
  jQuery(document).ready(function () {
    jQuery(".unitlist-zoom").colorbox({
      ...
      title: function () {
        ...
        if (filter) {
          return (
            '<a class="" id="from-image-filter" data-url=' +
            filter +
    ...
    jQuery(
      "a#filter, div#filter, a#from-image-filter, .qgroups-unit-filter"
    ).colorbox({
      href: function () {
        var url = jQuery(this).data("url");
        return url;
...
jQuery(document).ready(function () {
  jQuery(document).on("click", ".g_additional_toggler", function () {
    ...
    var rowsToTogle = jQuery(document).find(
      'tr[data-unit="' + currentUnit + '"]'
    );
    g_toggleAdditional("g_DetailTable1", opennedimage, closedimage);
    jQuery(rowsToTogle).toggleClass("hidden");
  });

  $(document).on("mouseenter mouseleave", "tr.g_collapsed", function (e) {
    let name = jQuery(this).attr("name");
    let el = jQuery(document).find("tr[name=" + name + "]");
    if (e.type == "mouseenter") {
      el.addClass("g_highlight_over");
    } else {
      el.removeClass("g_highlight_over");
    }
  });
  ...
  //   jQuery(document)
  //     .find("tr[name=" + name + "]")
  //     .toggleClass("g_highlight_over");
  // });
});

var opennedimage = "/content/laximo/com_guayaquil/assets/images/openned.gif";
var closedimage = "/content/laximo/com_guayaquil/assets/images/closed.gif";
jQuery(document).ready(function ($) {
...
function unitHl(el, type) {
  ...
  if (type == "in")
    jQuery('.g_highlight[name="' + name + '"]')
      .addClass("g_highlight_over")
      .removeClass("g_highlight");
  else
...
function g_getHint() {
  ...
  for (var i = 0; i < items.length - 1; i++) {
    ...
    str =
      str +
      "<tr><th align=right>" +
      jQuery("#" + jQuery(items[i]).attr("name")).text() +
      ":&nbsp;</th><td>" +
      txt +
      "</td></tr>";
  }
  ...
  for (var k = 0; k < note_items.length; k++) {
    ...
    str =
      str +
      "<tr><th align=right>" +
      jQuery(note_items[k]).find("span.name").text() +
      ":&nbsp;</th><td>" +
      txt +
      "</td></tr>";
  }
...
jQuery(document).on(
  "click",
  ".vehicle-modifications .show-more-wrapper",
  function () {
    var $this = jQuery(this);
...
jQuery(document).on(
  "click",
  ".vehicle-modifications .show-more-button",
  function () {
    var $this = jQuery(this);
