...
function start_table()
{
	...
	?>
	<TR CLASS=R18>
	<TD CLASS="R18C15" COLSPAN=16><SPAN STYLE="white-space:nowrap;max-width:0px;">Страница&nbsp;<?php echo $current_page; ?></SPAN></TD>
	<TD><SPAN></SPAN></TD>
	<TD></TD>
	</TR>
...
// ---------------------------------------------------------------------------------
//Вывести ИТОГО ПО ВСЕЙ НАКЛАДНОЙ
function end_all()
{
...
- количество строк не больше 4 (например, 1 позиция с наименованием на 4 строки)
*/
?>
...
//Тестовый запрос
$need_count_rows_for_test = 17;//Какое количество строк вывести для теста
...
while( $order_item = $order_items_query->fetch() )
{
	...
	if( $count_items == $pos_start )
	{
		...
		if( $current_page == 1 )
		{
			//Если количество позиций до 4, то, помещаемся на одной странице
			if( $elements_count_rows <= 4)
			{
				$pos_end_table = $elements_count_rows;
				//$pos_end_all = $elements_count_rows;
			}
			//Если позиций больше 4, но, до 16, то помещаемся на двух страницах
			else if( $elements_count_rows <= 16 )
			{
				$pos_start = 5;//Начало новой страницы с 5 позиции
				
				$pos_end_table = 4;//Завершение текущей страницы на 4 позиции
				
				$pos_end_all = $elements_count_rows;
			}
			else//Более 16
			{
				$pos_start = 16;//Начало новой страницы с 16 позиции
				
				$pos_end_table = 15;//Завершение текущей страницы на 15 позиции
				
				//$pos_end_all = $elements_count_rows;//Это еще не известно
			}
		}
		else//Находимся на второй и последеющих страницах
		{
			if( $elements_count_rows <= 16 )
			{
				//Это может быть только если это вторая и последняя станица
				$pos_end_table = 0;
				$pos_end_all = $elements_count_rows;
			}
			//Если осталось менее 20 позиций, то умещаемся на данной странице
			else if( $elements_count_rows - $count_items <= 20 )
			{
				$pos_end_table = 0;
				$pos_end_all = $elements_count_rows;
			}
			//Осталось более 20, но, менее 33 - здесь не помещаемся, но уже знаем, что впереди только 1 страница
			else if( $elements_count_rows - $count_items <= 33 )
			{
				$pos_start = $count_items + 21;//Начало новой страницы
				
				$pos_end_table = $count_items + 20;//Завершение текущей страницы
				
				//$pos_end_all = $elements_count_rows;
			}
			else//Впереди еще более 33 позиций
			{
				$pos_start = $count_items + 33;//Начало новой страницы с 16 позиции
				
				$pos_end_table = $count_items + 32;//Завершение текущей страницы на 15 позиции
			}
		}
	...
	<TD CLASS="R22C3"><?php echo $order_item["article"];?></TD>
...
<TABLE style="width:100%; height:0px; " CELLSPACING=0 id="signatures_table">
<COL WIDTH=7>
<COL WIDTH=125>
<COL WIDTH=112>
<COL WIDTH=12>
...
<TR CLASS=R18>
<TD CLASS="R24C7" COLSPAN=2><SPAN STYLE="white-space:nowrap;max-width:0px;">М.П.</SPAN></TD>
