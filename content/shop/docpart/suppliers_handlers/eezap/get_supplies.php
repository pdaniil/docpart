...
class eezap_enclosure
{
	...
	public function __construct($article, $manufacturer, $storage_options)
	{
		...
		$brand			= $manufacturer[0]["manufacturer_id"];
		...
		try
		{
			...
			{
				$DocpartSuppliersAPI_Debug->log_api_request("Получение остатков по артикулу ".$article." и производителю ".$manufacturer[0]["manufacturer"]." (ID ".$manufacturer[0]["manufacturer_id"].")", "SOAP-вызов метода getFindByDetail() с параметрами:<br>".print_r($params,true), "См. ответ API после обработки", print_r($result, true) );
			}
