...
class tradesoft_enclosure
{
	...
	public function __construct($article, $manufacturers, $storage_options)
	{
		...
		for($i = 0; $i < count($manufacturers); $i++)
		{
			$request = array(
				...
				'container'	=> array(
					array(
						...
						'code'		=> $manufacturers[$i]["manufacturer_id"],
						'producer'	=> $manufacturers[$i]["manufacturer"]
					)
				)
			);
			...
			{
				$DocpartSuppliersAPI_Debug->log_api_request("Получение остатков по артикулу ".$article." и производителю ".$manufacturers[$i]["manufacturer"], 'https://service.tradesoft.ru/3/'."<br>Метод POST<br>Поля: ".print_r($request, true), $data, print_r(json_decode($data, true), true) );
			}
...
$ob = new tradesoft_enclosure($_POST["article"], json_decode($_POST["manufacturers"], true), $storage_options);
exit(json_encode($ob));
?>