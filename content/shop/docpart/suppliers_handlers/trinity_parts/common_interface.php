...
class trinity_parts_enclosure
{
	...
	public function __construct($article, $storage_options)
	{
		...
		for($b=0; $b < count($brands["data"]); $b++)
		{
			...
			if($DocpartSuppliersAPI_Debug->suppliers_api_debug)
			{
				$DocpartSuppliersAPI_Debug->log_simple_message("Получаем остатки по артикулу ".$article." и производителю ".$brands["data"][$b]["producer"]);
			}
			
			$items = $ws->searchItems($article, $brands["data"][$b]["producer"]);
