	...
	public function __construct($article, $manufacurers, $storage_options)
	{
		...
		foreach($manufacurers as $brand)
		{
			...
			try
			{
				...
				if($DocpartSuppliersAPI_Debug->suppliers_api_debug)
				{
					$DocpartSuppliersAPI_Debug->log_api_request("Получение остатков по артикулу ".$article." и производителю ".$brand['manufacturer']." (ID ".$brand['manufacturer_id'].")", "SOAP-вызов search с параметрами ".print_r($param, true), htmlentities(print_r($soap_am_result, true)), "Преобразование результата не требуется" );
				}
...
$ob = new autoimport31_enclosure($_POST["article"], json_decode($_POST["manufacturers"], true), $storage_options);
exit(json_encode($ob));
?>