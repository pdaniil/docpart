...
class avtosputnik_enclosure
{
	...
	public function __construct($article, $manufacturer, $storage_options)
	{
		...
		$brand_search = $manufacturer[0]["manufacturer_id"];
		...
		{
			$DocpartSuppliersAPI_Debug->log_api_request("Получение остатков по артикулу ".$article." и бренду ".$manufacturer[0]["manufacturer"]." (ID ".$manufacturer[0]["manufacturer_id"].")", $url, $execute, print_r(json_decode($execute, true), true) );
		}
