...
class ml_auto_by_enclosure
{
	...
	public function __construct($article, $manufacturers, $storage_options)
	{
        ...
        for($i=0; $i<count($manufacturers);$i++)//Цикл по массиву брэндов
        {
			...
			curl_setopt($ch, CURLOPT_POSTFIELDS, "LOGIN=$login&PASSWORD=$pass&ARTICLE=$article&BRAND=".$manufacturers[$i]["manufacturer"]."&SEARCH_TYPE=1");
			...
			{
				$DocpartSuppliersAPI_Debug->log_api_request("Получение остатков по артикулу ".$article." и производителю ".$manufacturers[$i]["manufacturer"], "https://www.ml-auto.by/webservice/Search/<br>Метод POST<br>Поля "."LOGIN=$login&PASSWORD=$pass&ARTICLE=$article&BRAND=".$manufacturers[$i]["manufacturer"]."&SEARCH_TYPE=1", $curl_result, print_r(json_decode($curl_result, true), true) );
			}
...
$ob = new ml_auto_by_enclosure($_POST["article"], json_decode($_POST["manufacturers"], true), $storage_options);
exit(json_encode($ob));
?>