<?php
//Скрипт для метода изменения количества товаров в корзине
...
if( $session_query->fetchColumn() > 0 )
{
	...
	$request_object["count_need"] = $params["count_need"];
	$request_object["id"] = $params["cart_record_id"];
	...
	$curl = curl_init();
	curl_setopt($curl, CURLOPT_URL, $DP_Config->domain_path."content/shop/order_process/ajax_change_count_need.php");
	...
	if( $curl_result["status"] == true )
	{
		...
		$answer = array();
		$answer["status"] = true;
		$answer["message"] = "Change count need";
